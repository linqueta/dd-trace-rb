...
RSpec.describe 'profiling integration test' do
  ...
  describe 'using the Pprof::Builder' do
    let(:builder) { Datadog::Profiling::Pprof::Builder.new(Datadog::Profiling::Pprof::Builder::CONVERTERS) }

    shared_context 'StackSample flush' do
      let(:stack_sample_flush) do
    ...
    context 'after adding flushes' do
      include_context 'StackSample flush'

      let(:flushes) { [stack_sample_flush] }

      before do
        flushes.each { |flush| builder.add_flush!(flush) }
      end

      describe 'building a profile' do
